 ...
 * 2. Uniformity: Paths using the same number (e.g. T20, D20) are prioritized.
 * 3. High Value Start: Paths starting with larger numbers are shown first.
 */
export const calculateCheckouts = (target: number, mode: GameMode): CheckoutPath[] => {
  ...
  return paths.sort((a, b) => {
    ...
    // 2. Uniformity (Same Number Priority)
    ...
    // 3. First Dart Value DESC (Big numbers first)
    ...
    // 4. If values identical, prefer higher multipliers for first dart (T20 > S20? Handled by value usually)
    // But e.g. S20 (20) vs D10 (20). Value is same.
    // Prefer higher multiplier (more elegant) or lower (safer)?
    // Usually T/D is "cleaner" than S.
